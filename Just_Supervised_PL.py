...
def train_net(net,
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                if (global_step) % (inference_step) == 0: # Should be changed if the condition that the n_train%batch_size != 0
                    ...
                    val1, val2, val3, val4, val5, val6, val7, val8, inference_time = eval_dice_IoU(net, SourceTest_loader, device, test_counter, save_test, num_classes = num_classes, save=False, Pyramid_Loss = True)
                    ...
                    if dataset_mode == 'csv':
                        ...
                        if 'Endovis' in project_name or 'Endometriosis' in project_name:
                            print("one domain")
                        else:   

                            val1_target, val2_target, val3_target, val4_target, val5_target, val6_target, val7_target, val8_target, inference_time_target = eval_dice_IoU(net, test_loader, device, test_counter, save_test, num_classes = num_classes, save=False, Pyramid_Loss = True)
    ...
    val1, val2, val3, val4, val5, val6, val7, val8, inference_time = eval_dice_IoU(net, test_loader, device, test_counter, save_test, num_classes = num_classes, save=False, Pyramid_Loss = True)
    ...
    try:
        for c in range(len(Categories)):      
            for LR in range(len(Learning_Rates_init)):
                ...
                elif 'Cataract' in project_name or 'MRI' in project_name:
                ...
                net = Net1(n_classes=num_classes, n_channels=3, Pyramid_Loss=True)
