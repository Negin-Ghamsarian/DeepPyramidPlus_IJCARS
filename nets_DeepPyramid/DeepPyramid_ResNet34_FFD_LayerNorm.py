...
from .NetModules_utils_FFD_LayerNorm import *




class DeepPyramid_ResNet34_FFD_LayerNorm(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True, Pyramid_Loss=False, init_shape = [512, 512]):
        super(DeepPyramid_ResNet34_FFD_LayerNorm, self).__init__()
        ...
        self.Backbone = ResNet34_Separate()
        
        self.firstUpSample = nn.Upsample(scale_factor=2)
        
        self.glob1 = PVF(input_channels=512, pool_sizes=[32, 9, 7, 5], dim=32)
        
        self.glob2 = PVF(input_channels=128, pool_sizes=[64, 9, 7, 5], dim=64)
        self.glob3 = PVF(input_channels=64, pool_sizes=[128, 9, 7, 5], dim=128)
        self.glob4 = PVF(input_channels=64, pool_sizes=[256, 9, 7, 5], dim=256)

        self.up1 = Up(512+256, 128, [3, 6, 7], [init_shape[0]//8, init_shape[1]//8], bilinear)
        self.up2 = Up(256, 64, [3, 6, 7], [init_shape[0]//4, init_shape[1]//4], bilinear)
        self.up3 = Up(128, 64, [3, 6, 7], [init_shape[0]//2, init_shape[1]//2], bilinear)
        self.up4 = Up(128, 32, [3, 6, 7], [init_shape[0], init_shape[1]], bilinear)

        self.outc = OutConv(32, n_classes)

        if self.Pyramid_Loss:
            print("Pyramid Loss activated in the network")
            self.mask1 = nn.Conv2d(128, 1, kernel_size=3, padding=1)
            self.mask2 = nn.Conv2d(64, 1, kernel_size=3, padding=1)
...
if __name__ == '__main__':
    ...
    model_without_PyramidLoss = DeepPyramid_ResNet34_FFD_LayerNorm(n_channels=3, n_classes=1, bilinear=True, Pyramid_Loss=False)
    ...
    model_with_PyramidLoss = DeepPyramid_ResNet34_FFD_LayerNorm(n_channels=3, n_classes=1, bilinear=True, Pyramid_Loss=True)
